...
. "$env:SCOOP\apps\scoop\current\lib\json.ps1"
. "$env:SCOOP\apps\scoop\current\lib\unix.ps1"
...
. "$env:SCOOP\apps\scoop\current\bin\checkver.ps1" -App $App -Dir $Dir -Update -SkipUpdated:$SkipUpdated -ThrowError:$ThrowError
...
hub diff --name-only | ForEach-Object {
    ...
    if ($Push) {
        Write-Host "Creating update $app ($version) ..." -ForegroundColor DarkCyan
        python "$PSScriptRoot/../makeindex.py"
        execute "hub add $manifest"
        execute "hub add README.md"
